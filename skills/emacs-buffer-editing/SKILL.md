...
type: instruction
tools: ["emacs_read", "emacs_eval_live", "read_file"]
capability: true
source: builtin
source-name: emacs
features: ["emacs"]
prompt-keywords: ["buffer", "region", "point", "mark", "window", "narrow", "selected text"]
...
Use `read_file` with `source=live-buffer` when a known file-visiting buffer's live text is needed,
including unsaved edits. Use `read_file` with `source=disk` only when the saved disk version is
the intended source of truth. Use `emacs_eval_live` for non-file buffers and
