...
| Locality | Tools | 契约 |
|----------+-------+------|
| ~local~ | ~emacs_read~、~emacs_eval~、~emacs_eval_live~、~read_tool_output~、child-agent tools、~web_search~ | 在 Emacs host 执行；~emacs_eval~ 只把 TRAMP project root 当作数据传给本地 child。 |
...
~magent-tools.el~ 通过单一 canonical catalog 暴露 16 个 ~gptel-tool~ ：
