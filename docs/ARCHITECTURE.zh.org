...
| ~project-process~ | ~bash~、~grep~ | 唯一允许在 project host 启动的工具；remote execution 显式使用 TRAMP process handler。 |

Execution locality 绝不只由 ambient ~default-directory~ 决定。
project-process executable 必须在 project host 上解析；失败就在该边界明确返回，
不会回退到本地主机或本地 home。受信任的 Action argv Step 与 Doctor process
