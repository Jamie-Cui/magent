...
这个结构让替换点保持可见。换 provider 应该局限在 ~magent-sampling-gptel.el~ 和 gptel 配置里；换 frontend 应该局限在 ~magent-agent-shell.el~、~magent-acp.el~ 和 ~magent-runtime-api.el~ 附近；Action registration、Workflow、Step 或 Invocation 的修改应该通过 ~magent-action.el~，isolated Action persistence 属于 ~magent-action-session.el~，交互 viewer 属于 ~magent-action-session-view.el~；其余持久化和恢复修改应该通过 ledger/session 模块；工具策略应该通过 permission 和 orchestrator 模块。

** TRAMP locality 契约

Remote project 只改变 resource 所在位置，不改变 Magent control plane
所在主机。agent-shell UI、进程内 ACP placeholder、gptel transport、runtime
queue、ledger/session persistence、Action control flow、Doctor、skill 安装、
child-agent coordination、~emacs_read~、~emacs_eval~、~emacs_eval_live~ 与
~web_search~ 都留在本机。Remote session key 的规范化不会探测 TRAMP
filesystem；agent-shell 渲染 remote file context 时也不会进行 image 或
containment 查询。

Canonical tool catalog 为每个工具声明唯一 locality：

| Locality | Tools | 契约 |
|----------+-------+------|
| ~local~ | ~emacs_read~、~emacs_eval~、~emacs_eval_live~、~read_tool_output~、child-agent tools、~update_plan~、~web_search~ | 在 Emacs host 执行；~emacs_eval~ 只把 TRAMP project root 当作数据传给本地 child。 |
| ~tramp-file~ | ~read_file~、~write_file~、~edit_file~、~glob~ | 本地 Elisp 调用 Emacs file API；remote resource 通过 TRAMP 访问，不启动 project-host process。 |
| ~project-process~ | ~bash~、~grep~ | 唯一允许在 project host 启动的工具；remote execution 显式使用 TRAMP process handler；search 选择 ~rg~ 或同主机的 ~git grep~。 |

Execution locality 绝不只由 ambient ~default-directory~ 决定。
project-process executable 必须在 project host 上解析；失败就在该边界明确返回，
不会回退到本地主机或本地 home。
~grep~ tool 只会在同一 project host 缺少 ~rg~ 时选择
~git grep --no-index --exclude-standard~，不会静默替换为 recursive/ignore
语义不够 portable 的普通 ~grep~。Git backend 使用 POSIX extended regex，并在
tool-result metadata 中记录 backend identity。受信任的 Action argv Step 与 Doctor process
probe 均为 local-only，并拒绝 remote working directory。这是 locality/failure
契约，不是 OS sandbox。
...
~magent-sampling.el~ 定义 provider-neutral request/event。 ~magent-sampling-gptel.el~ 负责一次 sampling request：调用 ~gptel-request~ ，再把 gptel callback 转成 Magent normalized events。这个 adapter 可以隐藏 gptel callback/FSM 细节，但主 loop 只消费 normalized events。Reasoning event 与 assistant text 分开保存；普通 agent turn 缺少最终 assistant text 时会明确失败；直接 sampler 保留各自允许空结果的契约，reasoning 不作为最终回答。

Thinking policy 在 adapter boundary 之前保持 provider-neutral。Core runtime 只使用
~auto~、~enabled~ 和 ~disabled~；只有存在可靠 wire mapping 的 backend 才翻译显式
mode。DeepSeek 使用原生 ~thinking.type~ request object。显式 disable 会抑制 reasoning
effort；未知映射会在 provider dispatch 前失败，而不是静默采用 provider default。

每次 provider continuation 都会开始一个新的 sample 边界。公开的 terminal result
只包含最终 assistant message；没有原生消息阶段的 provider 使用最后一个 sample 的 assistant text，ledger 则保留完整 turn transcript 和 tool
...
~AGENTS.md~、~magent-context-provider-functions~ 返回的可信 request-local context，
以及 capability/显式 instruction skills。最后追加短小的 runtime
...
Context provider 是受信任的本地 Elisp 扩展。每个 provider 接收 user prompt、request
context 和 project root，并可返回一个自包含的 system-context block。Provider 按配置
顺序运行；异常或非法返回值会记录日志并被忽略。用户消息、文件、buffer、日志、命令
输出、tool result、网页和 conversation history 仍只保持各自的数据角色；其中伪装成
prompt 或高优先级标签的文本不能授予权限，也不能把自己提升成 runtime instruction。
Prompt 规则只负责引导模型行为，真实的 tool availability、approval 与 permission
enforcement 仍由 tool runtime 和 orchestrator 负责。

Base prompt 只保留跨任务 coding 行为；tool schema 由 gptel 提供，Emacs-specific
流程通过 instruction skills 渐进式披露。~web_search~ 返回标题、URL、摘要与来源引用；
~web_open~ 抓取正文到会话私有的不可变快照，~web_find~ 在快照中查找。三者共用
~web_search~ 权限。默认搜索源为免密钥 Bing RSS，可选 Tavily 与 gptel 模型独立；
搜索源注册、依赖和限制见 README。
...
- ~update_plan~, ~web_search~, ~web_open~, ~web_find~
...
~magent-tool-result~；只有 provider boundary 会转换为文本，非结构化结果会被拒绝。
过大的 model-visible result 在中心位置截断，完整结果写入当前
session 私有、带 TTL/quota 的 spill，只能凭 opaque id 通过 ~read_tool_output~ 分页
读取。spill identity 同时包含精确 runtime scope 与 session id；分页使用从 1
开始的字符偏移，因此超长单行也能完整取回。~glob~ 使用有界的 event-loop slices。

~magent-tool-orchestrator.el~ 负责权限解析、必要时请求 approval、执行工具、写 audit，再把结果交回 loop。 ~magent-permission.el~ 按声明顺序选择第一个匹配规则；~*~ 只是普通的显式 glob 规则，没有隐式 fallback。无匹配规则时 default allow。
...
5. ~magent-runtime-queue.el~ 在该会话的执行槽空闲时启动 turn；其他会话（包括同一项目）可同时运行。
...
12. 如果 provider 返回空 assistant text 并明确完成， ~magent-agent.el~ 用 ~empty-completion~ metadata 将本轮标记为 failed，不再发起请求。
...
- bundled ~skills/~、用户目录 ~~/.emacs.d/magent/skills~，以及项目 ~.magent/skills/~ 下的 skills。
...
继承 frontend request context；它也可以冻结 request-local reasoning effort 和 thinking
mode。Content blocks 和 canonical Action metadata 由内部
持久化。高级受信任 package 可以拥有更长的异步 Invocation，但不会接触 ACP 或 queue
internals。内置 ~/explain~、~/fix~、~/init~、~/review~ 和 ~/test~
...
两者都不拥有第二套 registry 或 context 类型。~/doctor~ 同时提供
~magent-action-run-*~ wrapper，两种入口执行相同 Invocation lifecycle。它由
~magent-action-enabled-builtins~ 控制注册；Custom 变更会原子刷新 registry 和
frontend discovery。新 session
写入 ~magent-session-directory/actions~。旧 ~commands/~ 格式既不迁移、也不读取，
文件原样留在磁盘上。

~magent-action-run-doctor~ 是安全敏感的 direct-pipeline 场景。 ~magent-action-builtin-doctor.el~
...
- ~skill~ 是可复用、data-only 的模型说明，在普通 turn 中显式或自动选择；
  它不会转换成 Action。
- ~capability~ 是自动激活规则：由 ~modes~ 、~features~ 、~files~ 、~prompt-keywords~ 、~disclosure~ 和 ~risk~ 等 resolver metadata 描述什么时候为当前 turn 选择一个或多个 skills。只有 context 的匹配会作为可检查的 suggestion；active disclosure 还要求当前 user prompt 命中有词边界的 keyword。
...
- Provider 工作留在 gptel 和 ~magent-sampling-gptel.el~ 。
