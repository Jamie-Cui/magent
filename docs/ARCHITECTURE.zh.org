...
- bundled ~skills/~ 、用户 ~~/.emacs.d/magent/skills~ 、存在时兼容 legacy ~~/.emacs.d/magent-skills/~ 、以及项目 ~.magent/skills/~ 下的 skills。
...
- skill-backed capability 是内置默认能力的常见形态：某个 ~SKILL.md~ 声明 ~capability: true~ 后，resolver 会在当前 buffer 或 prompt 匹配时自动激活同名 skill。~init~ 、~review~ 、~fix~ 、~test~ 和 ~explain~ 这类 command-style built-in skills 仍然只是 skills，除非它们显式声明 ~capability: true~。
