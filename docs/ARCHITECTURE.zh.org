...
从功能面看，Magent 可以按四条主线使用。第一条是 coding assistant：读代码、搜代码、改文件、跑测试、解释错误、生成 commit/PR 风格总结。第二条是 Emacs runtime assistant：通过 ~emacs_eval~ 查询 buffer、mode、window、project、Magit、Org、LSP 等活状态。第三条是 agent workflow：用 skills、capabilities 和 child-agent jobs 固化或拆分任务。第四条是 isolated command workflow：Doctor 和 Memory 可以从 agent-shell slash 或 ~M-x magent-command-run-*~ 启动同一个 command spec，并把 durable ledger 与当前对话隔离。
