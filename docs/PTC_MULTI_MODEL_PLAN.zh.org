...
Session effort 和 thinking option 是用户对整个 turn 的显式 override，应用到所有
phase；它们不改变各 phase 的 backend/model。显式 disabled thinking 抑制 effort。
...
backend/model/temperature/top-p/effort/thinking 将替换为 per-phase route override 和当前 epoch
