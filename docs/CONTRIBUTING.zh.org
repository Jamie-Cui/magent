...
melpazoid recipe 通过 ~:defaults~ 包含 production libraries，并显式添加 ~prompt/~、~skills/~ 和 ~capabilities/~。这些 bundled runtime data 必须保留，因为 Magent 会在运行时解析这些路径。匹配 ~*-test.el~ 的文件应放在 ~test/~ 下；MELPA 会从 production package 中排除该模式。
