...
- agent-shell 0.66.1+
- package-lint（仅开发时需要）
- ripgrep 或 Git，用于 ~grep~ tool 测试；优先使用 ripgrep
...
- Provider transport 继续使用 ~magent-sampling-gptel.el~ 和 ~gptel-request~ 。
...
- 不要重新引入已移除的 agent-loop modules；provider-specific transport handling 只放在 ~magent-sampling-gptel.el~ 。
