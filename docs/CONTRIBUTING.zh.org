...
melpazoid recipe 包含 ~prompt/~ 和 ~skills/~ 。这些是 bundled runtime data；~magent-config.el~ 、~magent-skills.el~ 和内置 skill-backed capabilities 会在运行时解析这些路径，因此必须保留在 package recipe 里。
