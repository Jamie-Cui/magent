...
make clean      # 删除构建产物及 Harbor benchmark 容器/网络
make purge      # 进一步删除 benchmark Docker 镜像及 Harbor task cache
#+end_src
...
melpazoid recipe 显式包含 ~lisp/*.el~、~prompts/~、~skills/~ 和 ~capabilities/~。这些 production libraries 和 bundled runtime data 必须保留，因为 Magent 会在运行时解析它们。这里不能使用 ~:defaults~：production 模块 ~magent-command-test.el~ 会因匹配 ~*-test.el~ 而被 MELPA 排除。
