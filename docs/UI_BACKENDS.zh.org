...
或者直接使用 agent-shell。显式 registered command 也可以通过 ~magent-command~
...
invocation 则属于 ~magent-command.el~ 。
...
- ~magent-command.el~ 负责 registered slash/interactive command discovery 和
  invocation lifecycle； ~magent-command-session.el~ 提供 isolated durable
...
4. ACP 按 runtime session 的精确 scope 解析 registered slash input，并通过
   ~magent-command.el~ 调用；普通 prompt 直接进入 ~magent-runtime-api.el~ 。Command
