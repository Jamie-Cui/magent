...
也不能拥有 active 或 queued work。selected agent、model route、effort、thinking
mode 和 automatic capability 设置等稳定 frontend option 会被继承；session approval override、
...
Assistant message item 按顺序保留进度、工具和最终答复边界，reasoning 显示设置
不影响可见的进度说明。Runtime 的 plan-update 事件携带 entries，由 magent-acp
转换为原生 plan update；保存的 plan item 使用同一路径恢复，由 agent-shell
负责显示。取消或失败的工具在恢复时显示失败；普通 turn 没有最终答复时，pending
prompt 返回明确错误，不再隐藏地请求模型补写总结。
