#+title: Commands And Action Workflows
#+magent_lang: en
...
- *Slash commands* invoke Elisp-native Actions in the current agent-shell
  conversation. Most bundled prompt Actions declare one ordinary agent turn.
- *Interactive commands* are ~M-x~ wrappers over Actions that opt into
  interactive exposure.

The bundled package provides twelve slash commands by default. Doctor and the
three Memory Actions also expose ~M-x magent-action-run-*~ wrappers and use
isolated durable sessions.

Doctor and Memory are optional built-in Action groups. Customize
~magent-action-enabled-builtins~, or change it with ~setopt~, to enable or
disable either group at runtime:

#+begin_src elisp
;; Keep Doctor; disable memory-init, memory-refresh, and memory-clear.
...
invocation. The ~memory~ entry controls only the three memory-management
Actions. Profile-memory injection remains independently controlled by
...
| ~/memory-init~ | Build Emacs profile memory | Uses an isolated Action session and confirmation |
| ~/memory-refresh~ | Refresh managed profile memory | Preserves user notes in an isolated Action session |
...
Returns the effective catalog view for the current session agent. Each row
shows the permission key, final decision, whether the tool is exposed, the
rule or session-override source, resource sub-rules, approval policy, and
whether execution happens in the host, a child Emacs, or the live Emacs.
...
- ~magent-workflow-agent-turn~ runs an intermediate model turn and returns its text
  by default. It accepts ~:agent~, ~:skills~, ~:buffers~,
  ~:append-argument-p~, ~:tools~, ~:request-context~,
  ~:resource-blocks~, and ~:result~.
- ~magent-workflow-answer~ accepts the same model options but is terminal. It
...
- ~magent-workflow-process~ runs only a non-empty argv list of strings. It
  ...
  ~magent-action-process-result~ including stderr, exit status, duration, and
  timeout state. Process Steps are local-only: a remote captured or explicit
  directory fails before process creation. Use an agent Step with the ~bash~
  tool when an Action intentionally needs project-host execution.
- ~magent-workflow-callback~ adapts an existing asynchronous Elisp API. Its
...
below. Both surfaces execute the same Action spec. Every run gets an isolated
...
|--------------------------------+----------+--------------|
| ~magent-action-run-memory-init~       | Build initial Emacs profile memory | Uses the current gptel provider when ~magent-memory-use-llm~ is non-nil |
| ~magent-action-run-memory-refresh~    | Refresh managed profile memory | Uses the current gptel provider when ~magent-memory-use-llm~ is non-nil |
| ~magent-action-run-memory-clear~      | Deactivate and clear managed memory | Local operation; does not request an LLM summary |
...
~M-x magent-action-run-memory-init~ discovers bounded Emacs configuration sources,
shows a scan plan for approval, and writes the managed profile memory file.
~M-x magent-action-run-memory-refresh~ repeats the scan while preserving user notes.
...
~M-x magent-action-run-memory-clear~ deactivates the managed profile and clears its
