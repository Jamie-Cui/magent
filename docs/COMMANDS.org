...
- ~magent-workflow-agent-turn~ runs an intermediate model turn and returns its text
  ...
  ~:append-argument-p~, ~:tools~, ~:effort~, ~:thinking~, ~:request-context~,
...
~M-x magent-action~ asks before loading unapproved files. Approval covers the
...
|-----------------------------------+---------|
| ~magent-action~                | Choose and run an Action using standard minibuffer completion |
...
~M-x magent-action~ lists Actions exposed interactively in the current scope,
with descriptions and selection history. It uses the same completion UI as
~M-x~, including installed completion frontends such as Vertico/posframe.
Choose an Action and press ~RET~ to run it. Use ~C-u M-x magent-action~ to
supply an argument. Tool permissions follow the existing agent/session rules.
You can bind this entry point in your Emacs configuration, for example:
...
- Use Doctor when sanitized diagnosis needs its own durable session; choose
  slash or M-x according to the current UI.
