...
Org ID, repository path, analyzed Git commit, filename, and atomic update.
...
instruction skill through the recommended agent-shell backend. The legacy UI
offers the equivalent ~M-x magent-ui-run-skill-command~ command.
...
- Use ordinary Magent prompt commands such as ~magent-dwim~,
  ~magent-prompt-region~, and ~magent-ask-at-point~ for free-form requests rather
