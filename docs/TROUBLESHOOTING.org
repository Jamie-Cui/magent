...
MELPA's ~:defaults~ spec deliberately excludes test-file patterns, so production
libraries must not use ~*-test.el~ or ~*-tests.el~ names.
...
(magent :fetcher github :repo "Jamie-Cui/magent"
        :files (:defaults "prompt" "skills" "capabilities"))
#+end_src
