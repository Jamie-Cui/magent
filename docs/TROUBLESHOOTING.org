...
Magent needs every production library plus bundled runtime data after
packaging.
...
- If the continuation completes without assistant text, inspect the turn result for ~(:reason empty-completion)~. Magent does not issue a recovery request; reasoning-only content remains separate from assistant text.
...
*Solution:*
- For bash: Verify Bash is installed and ~magent-bash-program~ resolves to it.
  In a TRAMP project it must resolve on the remote project host; Magent does
  not fall back to local execution.
  Commands run without errexit and with pipefail.  Use ~&&~ or explicit ~set -e~
  when fail-fast sequencing is required.  Output-limiting pipelines such as
  ~rg ... | head~ can fail on SIGPIPE; prefer native limit options.
- For ~emacs_read~: verify that the requested fixed operation and its target
  are valid and that the origin buffer still exists.
- For ~emacs_eval~: check syntax and child-process diagnostics. It starts one
  fresh ~emacs -Q --batch~ process per call and cannot see live buffers or the
  user's loaded packages.
- For ~emacs_eval_live~: treat the timeout as best effort only. A blocking C
  primitive or process-wide failure may still hang or crash the live Emacs.
- For grep: Search prefers ripgrep (~which rg~) and falls back to
  ~git grep --no-index --exclude-standard~ (~which git~) on the project host.
  If neither executable exists there, the tool fails explicitly.

*** TRAMP project startup or a tool call hangs
*Problem:* Starting Magent from a remote file/Dired buffer or using a project
tool blocks Emacs.

*Diagnosis:*
1. Verify the same directory works with ordinary TRAMP file operations.
2. Check ~*magent-log*~ and ~*Messages*~ for the exact operation.
3. Confirm only ~bash~ or ~grep~ is expected to start a project-host process.

*Solution:*
- Keep the ACP placeholder, Actions, Doctor, and ~emacs_eval~ local; a remote
  process in those paths is a bug.
- Install ~bash~ plus either ~rg~ or Git on the remote project host for their
  respective ~project-process~ tools.
- Remote file tools still use synchronous Emacs/TRAMP file APIs. A broken SSH
  transport can therefore block an explicit file operation; repair or abort
  the TRAMP connection before retrying.

*** Elisp returned by the model hangs or crashes Emacs
*Problem:* An arbitrary form does not terminate, signals a fatal error, or
calls ~kill-emacs~.

*Diagnosis:*
1. Run ~/authority~ and confirm which eval boundary was selected.
2. Prefer ~emacs_read~ for live buffers, modes, symbols, bindings, hooks, and
   project state.
3. Confirm ordinary arbitrary code used ~emacs_eval~, whose result metadata
   says ~execution=child-process~.

*Solution:*
- Keep arbitrary evaluation on ~emacs_eval~. The parent owns the timeout and
  deletes the disposable child on timeout or cancellation.
- Use ~emacs_eval_live~ only when the task truly requires live package or
  buffer mutation. Every invocation requires a fresh approval; neither
  ~magent-bypass-permission~ nor a stored session allow suppresses it.
- This boundary contains Emacs-process failure, not host authority. Evaluated
  code still runs as the current user and can access files, processes, and the
  network.
...
The bypass does not suppress once-only approval for ~emacs_eval~ or
~emacs_eval_live~, and an agent-level deny still keeps a tool unavailable.
...
If issues persist:
1. Run ~M-x magent-action-run-doctor~ and save output
