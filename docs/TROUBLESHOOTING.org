...
- For bash: Verify Bash is installed and ~magent-bash-program~ resolves to it.
  Commands run with errexit and pipefail, so use conditionals or ~|| true~ only
  when a nonzero status is intentionally accepted.  Output-limiting pipelines
  such as ~rg ... | head~ can fail on SIGPIPE; prefer native limit options.
