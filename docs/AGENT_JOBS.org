...
#+magent_lang: en
#+magent_alt_url: /AGENT_JOBS.zh.html
...
4. Completion, failure, and close transitions notify wait observers and update
...
The provider boundary remains ~gptel-request~ through ~magent-sampling-gptel.el~. Magent owns orchestration, tool dispatch, persistence, abort behavior, and child-job coordination.
...
- backend/model names
- temperature, top-p, reasoning effort, and thinking mode
...
- live Emacs buffers
- isolated ~emacs_eval~, trusted ~emacs_read~, and explicit ~emacs_eval_live~
- agent-shell UI
