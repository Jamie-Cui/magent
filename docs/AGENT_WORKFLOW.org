...
Session JSON schema v6 writes both:
...
Loading requires the exact current schema, including ~snapshot + journal~.
Unknown fields, missing nested ledger fields, and other schema versions are
