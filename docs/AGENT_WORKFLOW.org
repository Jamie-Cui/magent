...
10. If a post-tool continuation completes with empty assistant text, Magent
    records the empty assistant message and completes the turn.  The empty
    message is preserved in the transcript but omitted from later prompt reuse.
...
13. ~magent-max-sampling-requests~ is an optional compatibility safety guard,
    ...
    this guard is explicitly enabled, Magent fails the turn after the limit
    instead of forcing a synthetic no-tool final answer.
