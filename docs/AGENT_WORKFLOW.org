...
session ~messages~ remain as a derived projection for gptel prompt
...
Older ~messages~ are kept as a projection for gptel prompt reuse and
migration. On load, Magent prefers replaying
~snapshot + journal~; if
...
requests dispatch registered slash commands through ~magent-command.el~ and
...
~magent-command~ invocation lifecycle without creating another conversation
...
- Codex rollout history stores response items and turn context items.
  Magent stores a ledger ~snapshot + journal~; ~messages~ is the only
  derived projection retained for gptel prompt reuse and migration.
