...
The melpazoid recipe explicitly includes ~lisp/*.el~, ~prompt/~, ~skills/~,
and ~capabilities/~. Those production libraries and bundled runtime data must
stay in the package recipe because Magent resolves them at runtime. Do not use
~:defaults~ here: MELPA would exclude the production ~magent-command-test.el~
module because its filename matches ~*-test.el~.
