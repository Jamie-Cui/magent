...
- ripgrep or Git (for grep tool tests; ripgrep is preferred)
...
The melpazoid recipe explicitly includes ~lisp/magent*.el~, ~prompts/~, and
~skills/~. Those production libraries and bundled runtime
...
- Keep using ~magent-sampling-gptel.el~ and ~gptel-request~ for provider transport
...
- Do not reintroduce removed agent-loop modules; add provider-specific transport handling only in ~magent-sampling-gptel.el~
