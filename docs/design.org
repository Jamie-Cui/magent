...
1. User invokes =magent-prompt= (=C-c m p=) → minibuffer input
...
| explore    | subagent         | Fast codebase exploration (read/grep/glob/bash)  |
| general    | subagent (hidden)| Multi-step task execution (no delegation)        |
| compaction | primary (hidden) | Session summarisation                            |
| title      | primary (hidden) | Conversation title generation                    |
| summary    | primary (hidden) | PR-style summaries                               |
...
| =magent-enable-logging=    | =t=                              | Log requests to =*magent-log*=       |
