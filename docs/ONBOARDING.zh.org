...
- ~magent-command-session.el~ ：isolated command 的持久化、ledger、viewer 与取消。
...
- ~magent-command.el~ ：公开的分层 slash-command registry 和 invocation lifecycle。
- ~magent-runtime.el~ ：静态初始化，以及 agents、skills、commands、capabilities 的 project-local overlay activation。
...
trusted command 或 canonical catalog 中的一等 tool；capability resolver 根据上下文
...
command lifecycle，而不是另一套 UI。

*关键文件：*

- ~magent-agent-shell.el~ ：Magent agent-shell config 和 prompt routing。
- ~magent-acp.el~ ：in-process ACP adapter。
- ~magent-command.el~ ：registered slash/interactive command discovery 和 invocation。
- ~magent-command-session.el~ ：isolated command persistence 和 cancellation。
...
- 需要可复用 instruction，并且可以被显式或自动选择时，用 ~skill~；
  ~/$name~ 是 ACP projection，~default-prompt~ 只额外提供 legacy
  ~/name~ 兼容。
...
~magent-command.el~ 。关键点：agent-shell 是唯一受支持的会话式 frontend；显式 ~M-x~
command wrapper 复用 command invocation lifecycle，共享 runtime 行为仍留在 runtime
API。
...
- ~magent-command.el~ ：trusted package 如何注册显式 slash 或 interactive action。
- ~magent-command-session.el~ ：isolated command invocation 如何持久化、报告 progress 和取消。
...
- *Interactive help:* ~M-x magent-command-run-doctor~
