...
- ~magent-tools.el~ ：单一 canonical catalog 中的 17 个 ~gptel-tool~ structs，包括显式 disk/live-buffer source 的 read、revision-checked write/edit、grep/glob、bash、emacs_read、隔离与 live eval、tool-result pagination、child-agent coordination、update_plan 和 web_search。
...
- *magent-tools.el* — 17 个 ~gptel-tool~ implementation 的 canonical catalog
...
*复杂原因：* 17 个 tool implementation 各自有 side effects、process lifecycle、revision contract、spill retention、child-agent runtime state 和 error handling。~emacs_read~ 使用捕获的 origin buffer，~emacs_eval~ 使用一次性 child process，~emacs_eval_live~ 刻意跨入主进程。
