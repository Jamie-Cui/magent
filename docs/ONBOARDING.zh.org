...
lazy initialization；agent registry、skills 等完整初始化按需执行。
...
command-like actions 运行，例如 built-in ~init~ 用于初始化或刷新项目根目录
~AGENTS.md~ 。
