...
~docs/UI_BACKENDS.org~ 中的不受支持 legacy-file 边界。
...
~magent-runtime-api.el~ 。关键点：agent-shell 是唯一受支持的交互体验。只有在移除或
隔离 unsupported compatibility code 时才读 ~magent-ui.el~ 和
~magent-ui-legacy.el~ 。
...
~magent-runtime-api.el~ 和 ~magent-runtime-queue.el~ 。Unsupported compatibility
code 仍保留在 ~magent-ui.el~ 和 ~magent-ui-legacy.el~ 。

*修改建议：* Frontend-neutral 行为放 ~magent-runtime-api.el~ ，ACP conversion 放
~magent-acp.el~ ，agent-shell-specific 行为放 ~magent-agent-shell.el~ 。不要向不受支持的
UI 文件增加用户入口。
