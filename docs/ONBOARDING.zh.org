...
- *要求：* Emacs 29.1+，gptel，compat，yaml，acp，agent-shell；project search 需要 ripgrep 或 Git
...
*职责：* 第一次调用受支持的 agent-shell 命令时， ~magent--ensure-initialized~ 触发
lazy initialization；agent registry、skills、slash commands 和 capabilities 等完整
初始化按需执行。
...
- ~magent-action-session.el~ ：isolated Action 的持久化、ledger 与取消。
- ~magent-action-mode-line.el~ ：可选的 active Action 计数和当前 Step tooltip。
- ~magent-action-session-view.el~ ：isolated Action session 的列表与查看。
- ~magent-action-builtin-doctor.el~ ：受信任只读 probes 和一次脱敏后的无工具分析请求。
...
- ~magent-runtime-queue.el~ ：会话内 FIFO、会话间并发的 runtime queue 和 session-scoped cancellation。
...
- ~magent-tools.el~ ：单一 canonical catalog 中的 19 个 ~gptel-tool~ structs，包括显式 disk/live-buffer source 的 read、revision-checked write/edit、grep/glob、bash、emacs_read、隔离与 live eval、tool-result pagination、child-agent coordination、update_plan、web_search、web_open 和 web_find。
...
*职责：* Tools 提供具体动作；skills 只提供 data-only instructions；可执行扩展使用
trusted Action 或 canonical catalog 中的一等 tool；capability resolver 根据上下文
自动激活少量 instruction skills；approval system gate dangerous operations。
...
- ~magent-sampling-gptel.el~ ：薄的 ~gptel-request~ sampling adapter。

*职责：* ~magent-agent-loop.el~ 消费 normalized LLM events，记录 assistant/tool state，经过 ~magent-tool-orchestrator~ 分发工具，并在 model-visible tool output 记录完成后返回 continuation outcome。 ~magent-agent-run-turn~ 负责 provider-native tool continuation，明确拒绝没有最终答复的普通 turn，并在可选 sampling limit 处直接失败。Reasoning 与 assistant text 分开保存。 ~magent-sampling-gptel.el~ 仍然调用 ~gptel-request~ ；Magent 不重写 provider transport。

** 第 6 层：受支持的 Frontend

agent-shell 是唯一受支持的会话式 frontend。显式 ~M-x~ command wrapper 使用相邻的
Action lifecycle，而不是另一套 UI。

*关键文件：*

- ~magent-agent-shell.el~ ：Magent agent-shell config 和 prompt routing。
- ~magent-acp.el~ ：in-process ACP adapter。
- ~magent-action.el~ ：Action Workflow、registry 和 slash/interactive projections。
- ~magent-action-session.el~ ：isolated Action persistence 和 cancellation。
- ~magent-action-session-view.el~ ：isolated Action session listing 和 inspection。
...
command 交给 ~magent-action.el~，把 scope-aware instruction skill descriptor
投影为 ~/$name~，并把该 skill selection 或普通 prompt 直接提交
~magent-runtime-api.el~。Prompt request 会保持 pending，直到对应普通 turn 或 Action
...
根到请求相关本地文件逐层适用的 ~AGENTS.md~、
~magent-context-provider-functions~ 返回的可信 context，以及显式选择或 capability
自动选择的少量 instruction skills。发现过程不会越过 canonical project
root，并受 ~magent-project-instructions-max-bytes~ 限制；把该选项设为 nil 可关闭。
所有 agent-loop prompt（包括 custom 和 utility agents）最后都会追加 runtime trust
policy。Context provider 是可信本地 Elisp，但它嵌入的 supporting data 不能覆盖当前
...
- 需要 executable trusted Elisp 时，用 ~Action~；它可以投影成 slash command、
  interactive command 或两者。
- ~command~ 只表示 frontend/protocol projection，不是领域对象。
- 需要可复用 instruction，并且可以被显式或自动选择时，用 ~skill~；
...
从 ~magent.el~ 了解 package initialization，再读 ~magent-agent-shell.el~ 中受支持的
interactive entry points。

** Step 2：UI 边界

读 ~docs/UI_BACKENDS.org~ ，再读 ~magent-agent-shell.el~ 、 ~magent-acp.el~ 和
~magent-runtime-api.el~ 。跟踪 slash 或 interactive command 时再读
~magent-action.el~ 。关键点：agent-shell 是唯一受支持的会话式 frontend；显式 ~M-x~
command wrapper 复用 Action Invocation lifecycle，共享 runtime 行为仍留在 runtime
API。
...
2. ~magent-action.el~ / ~magent-skills.el~ ：解析 registered slash command 或
   scope-aware ~/$skill~ descriptor。
3. ~magent-runtime-api.el~ / ~magent-runtime-queue.el~ / ~magent-ledger.el~ ：记录、排队并启动普通或 Action-owned turn。
4. ~magent-agent.el~ ：构造 prompt，选择 agent、skills、capabilities 和 tools。
5. ~magent-sampling-gptel.el~ / ~magent-agent-loop.el~ ：通过 gptel sampling，并负责 normalized continuation outcomes。
6. ~magent-tool-orchestrator.el~ / ~magent-tools.el~ ：解析权限并执行 tool implementations。
...
- ~magent-action.el~ ：trusted package 如何注册显式 slash 或 interactive action。
- ~magent-action-session.el~ / ~magent-action-session-view.el~ ：isolated Action Invocation 如何持久化、报告 progress、取消和渲染。
...
- ~read_file~ ：必须指定 ~source=disk~ 或 ~source=live-buffer~，并返回写入所需的 revision。
- ~emacs_read~ ：对 live Emacs 执行固定、受限的查询。
- ~emacs_eval~ ：每次在新的 child Emacs 中执行任意 Elisp，并要求 once-only approval。
- ~emacs_eval_live~ ：在 live Emacs 中执行任意 Elisp；危险、逐次确认，受限内置 agent 不暴露。
- ~read_tool_output~ ：凭 opaque id 在当前 session 内分页读取 spill。
- ~spawn_agent~ / ~send_agent_message~ / ~wait_agent~ / ~list_agents~ / ~close_agent~ ：durable child-agent jobs。
- ~web_search~、~web_open~、~web_find~：可配置搜索源（默认免密钥 Bing RSS）、原文快照与确定性查找；配置见 README。
...
** 核心入口与配置
- *magent.el* — package 入口、公开 autoload 与 lazy runtime bootstrap
- *magent-log.el* — UI-neutral logging sinks 与 headless fallback
- *magent-prompt.el* — 基于 manifest 的 prompt 加载与 placeholder 渲染
- *magent-config.el* — UI-neutral runtime 与 feature 配置
- *magent-json.el* — JSON-safe serialization helpers
- *magent-redaction.el* — Magent-owned、fail-closed 的 outbound redaction
- *magent-project-instructions.el* — 有界、按 scope 发现 ~AGENTS.md~ 并注入 prompt

** Protocol、状态与持久化
- *magent-protocol.el* — wire protocol 类型与 normalized runtime events
- *magent-lifecycle-events.el* — structured lifecycle event sinks 与 context helpers
- *magent-ledger.el* — canonical thread/turn/item 状态机、journal 与 snapshot
- *magent-session.el* — ledger projections、scoped JSON persistence 与 spill storage
- *magent-agent-job.el* — durable child-agent job 状态、runtime registry 与 JSON shape
- *magent-audit.el* — tool decision 的 live-buffer 或 JSONL audit records
...
- *magent-runtime-queue.el* — per-session execution queue 与 session-scoped cancellation
...
** Actions
- *magent-action.el* — Workflow DSL、managed Steps、layered registry 与 Invocation lifecycle
- *magent-action-controls.el* — ~/compact~ 等 core current-session controls
- *magent-action-skills.el* — scope-aware ~/skills~ Action projection
- *magent-action-builtins.el* — prompt Action definitions 与 built-in registration
- *magent-action-session.el* — isolated Action persistence、ledger 与 cancellation
- *magent-action-mode-line.el* — 可选的 active Action 计数与当前 Step tooltip
- *magent-action-session-view.el* — isolated Action session 列表与查看
- *magent-action-builtin-doctor.el* — trusted probes 与一次脱敏、无工具分析
...
** Tools、Skills 与 Capabilities
- *magent-web.el* — 搜索源注册、异步 HTTP 与来源快照
- *magent-web-page.el* — HTML/text/PDF 提取、分页和查找
- *magent-tools.el* — 19 个 ~gptel-tool~ implementation 的 canonical catalog
...
** 受支持的 Frontend
- *magent-acp.el* — in-process ACP adapter
- *magent-agent-shell.el* — Magent agent-shell 配置与隔离的 context compatibility

** CI 与 Package Data
- *.github/workflows/test.yml* — byte-compilation、unit tests 与 deterministic live smoke tests
- *.github/workflows/coverage.yml* — batch ~testcover~ 与 coverage artifact upload
- *.github/workflows/melpazoid.yml* — MELPA-style package checks
- *test/coverage.el* — ~make coverage~ 使用的 coverage runner

Magent 除 Elisp 外还打包 runtime data。~prompts/~ 与 ~skills/~ 必须继续包含在
melpazoid/MELPA recipe 中；所有 bundled Org prompt 也必须在
~prompts/manifest.txt~ 中恰好出现一次。此文件地图应始终与
~source-files.txt~ 中的所有 production files 对齐。
...
*复杂原因：* 受支持的 frontend 横跨 ~magent-agent-shell.el~ 、 ~magent-acp.el~ 、
~magent-runtime-api.el~ 和 ~magent-runtime-queue.el~ 。

*修改建议：* Frontend-neutral 行为放 ~magent-runtime-api.el~ ，ACP conversion 放
~magent-acp.el~ ，agent-shell-specific 行为放 ~magent-agent-shell.el~ 。
...
*复杂原因：* 19 个 tool implementation 各自有 side effects、process lifecycle、revision contract、spill retention、child-agent runtime state 和 error handling。~emacs_read~ 使用捕获的 origin buffer，~emacs_eval~ 使用一次性 child process，~emacs_eval_live~ 刻意跨入主进程。

*修改建议：* child-eval cleanup 和 host-owned timeout 必须可靠；两个任意 eval tool 的 once-only approval、spill 的 session isolation、写入的 revision validation、child-agent status persistence 和 parent/child request-context inheritance 都不能弱化。

** 6. ~magent-session.el~

*复杂原因：* 它负责 project scoping、global fallback、JSON persistence、ledger-driven UI restoration、child-agent job persistence 和 history trimming。

*修改建议：* session directory 由 project root SHA1 派生。 ~agent-jobs~ 需要为 child transcript inspection 保持无损。
...
Magent 除 Elisp 外还会打包 ~prompts/~ 与 ~skills/~ runtime data。每个 bundled Org
prompt 必须在 ~prompts/manifest.txt~ 中恰好出现一次；unit test 会把 manifest 与目录
实际内容对照。新增 prompt layer 时同时更新 manifest 和 MELPA/melpazoid package data
recipe。
...
- *Tool-use:* ~"帮我看下 emacs 里面有多少 buffer"~ ，验证 ~emacs_read~ tool。
- *Multi-step:* ~"帮我在 emacs 里面打开 magent 的 magit buffer"~ ，验证 chained execution。

检查当前 Magent agent-shell buffer、 ~*magent-log*~ 和 ~*Messages*~ 是否有错误。
...
请使用 ~read~、~write~、~edit~、~agent~ 这类 permission group key，并使用上面的
mapping 格式。未知字段、未知 permission key 和其他 shape 会直接报错。
