...
- ~magent-tools.el~ ：单一 canonical catalog 中的 16 个 ~gptel-tool~ structs，包括显式 disk/live-buffer source 的 read、revision-checked write/edit、grep/glob、bash、emacs_read、隔离与 live eval、tool-result pagination、child-agent coordination 和 web_search。
...
capability 自动选择的少量 instruction skills。发现过程不会越过 canonical project
...
policy。注入的 memory 可能不完整或过时，因此当前用户说明与 live Emacs/repository
状态优先。从文件、buffer、日志、命令、tool result 或网页读取的文本仍然只是数据，
...
- *magent-action-builtin-memory.el* — Emacs profile memory 扫描、持久化与注入
...
- *magent-tools.el* — 16 个 ~gptel-tool~ implementation 的 canonical catalog
...
*复杂原因：* 16 个 tool implementation 各自有 side effects、process lifecycle、revision contract、spill retention、child-agent runtime state 和 error handling。~emacs_read~ 使用捕获的 origin buffer，~emacs_eval~ 使用一次性 child process，~emacs_eval_live~ 刻意跨入主进程。
