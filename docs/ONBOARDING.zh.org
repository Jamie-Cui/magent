...
- ~magent-tools.el~ ：单一 canonical catalog 中的 15 个 ~gptel-tool~ structs，包括 disk/live-buffer read、file write、repository summary writer、grep/glob、bash、emacs_eval、child-agent coordination 和 web_search。
...
*复杂原因：* 15 个 tool implementation 各自有 side effects、timeout、child-agent runtime state 和 error handling。 ~read_file~ 与 ~read_buffer~ 必须保持 disk/live-buffer 语义分离， ~emacs_eval~ 要在正确 buffer context 中执行。
