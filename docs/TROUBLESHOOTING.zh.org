...
*原因：* MELPA-style packaging 只复制 recipe 匹配的文件。Magent 打包后仍然需要 production libraries 和 bundled runtime data。MELPA 的 ~:defaults~ 会排除 test-file patterns，包括 production 模块 ~magent-command-test.el~。
...
显式 glob 提供全部 production Elisp files，包括 ~magent-command-test.el~；显式目录提供 package 通过相对路径加载的 runtime data。ERT 源码应放在 ~test/~ 下，使其不进入 production package。
...
- bash：确认已安装 Bash，且 ~magent-bash-program~ 能解析到它。命令默认启用
  errexit 和 pipefail；只有明确接受非零状态时才使用条件判断或 ~|| true~ 。
  ~rg ... | head~ 一类限流 pipeline 可能因 SIGPIPE 失败，应优先使用工具原生
  limit 参数。
...
1. 运行 ~M-x magent-command-run-doctor~ 并保存输出。
