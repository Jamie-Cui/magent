...
*原因：* MELPA-style packaging 只复制 recipe 匹配的文件。Magent 打包后仍然需要 production libraries 和 bundled runtime data。MELPA 的 ~:defaults~ 会排除 test-file patterns，因此 production library 不应使用 ~*-test.el~ 或 ~*-tests.el~ 文件名。
