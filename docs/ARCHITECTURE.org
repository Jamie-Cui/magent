...
~magent-ui.el~, ~magent-ui-legacy.el~, and legacy UI files covered by the naming
rules in ~docs/UI_BACKENDS.org~ are retained unsupported code, not alternative
frontends. They must not become dependencies of the agent-shell runtime path.
...
- Keep supported frontend work in ~magent-agent-shell.el~ and out of the
  unsupported UI files.
