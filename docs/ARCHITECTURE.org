...
The default UI backend is ~agent-shell~. ~magent-agent-shell.el~ registers the Magent agent-shell configuration, ~magent-acp.el~ implements the in-process ACP adapter, and ~magent-runtime-api.el~ receives prompt submissions from UI backends. ACP text and resource blocks are normalized separately: the full structured input is persisted in turn metadata and reconstructed as user-role model context, while local ~file://~ resources contribute scoped file paths for project-instruction and capability resolution. ~magent-runtime-queue.el~ currently runs one active turn globally, while preserving session-scoped cancellation and queue bookkeeping.
...
Magent also has an Elisp-level internal command registry in ~magent-command.el~.
Commands registered with ~magent-command-register~ are invoked by explicit
~magent-run-*~ user commands, create isolated internal sessions under
~magent-session-directory/internal~, and may use either direct pipeline runners
or the normal Magent agent loop. They are intentionally separate from
agent-shell slash commands, whose command surface remains skill-backed.

~magent-run-doctor~ is the security-sensitive direct-pipeline case.  Trusted,
...
- A skill-backed capability is the common built-in case: a ~SKILL.md~ declares
  ...
  Command-style built-in skills such as
  ~init~, ~review~, ~fix~, ~test~, ~summarize~, and ~explain~ remain skills only unless they
