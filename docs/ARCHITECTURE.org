...
Legacy ~messages~ remain as a compatibility projection and migration input.
...
old-session import. Oversized model-visible results are truncated centrally;
the full value is stored in a private, TTL/quota-bounded session spill and may
be paged only with its opaque id through ~read_tool_output~. ~glob~ traverses in
bounded event-loop slices.
...
~/explain~, ~/fix~, ~/init~, ~/review~, ~/summarize~, and ~/test~ prompt Actions
...
execute the same Invocation lifecycle. New sessions are stored under
