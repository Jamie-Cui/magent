...
~magent-tools.el~ owns one canonical catalog of 15 ~gptel-tool~ structs:

- ~read_file~, ~read_buffer~, ~write_file~, ~write_repo_summary~, ~edit_file~
- ~grep~, ~glob~, ~bash~
- ~emacs_eval~
- ~spawn_agent~, ~send_agent_message~, ~wait_agent~, ~list_agents~, ~close_agent~
- ~web_search~

~read_file~ always reads saved disk state. ~read_buffer~ reads an existing
file-visiting buffer, including unsaved changes, without opening a file or
falling back to disk. Both use the ~read~ permission key. File writes reject
...
permission keys. Ordinary requests select ~:all~; command agent Steps pass an
...
boundary renders result text, and persisted legacy strings migrate only during
old-session import. ~glob~ traverses in bounded event-loop slices.
...
Executable extensions belong in trusted Elisp commands or the first-class tool
...
- A ~skill~ is reusable, data-only model instruction. The legacy
