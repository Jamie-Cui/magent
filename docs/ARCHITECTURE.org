...
Slash commands are a separate Elisp-native extension layer. Definitions in
~magent-command.el~ are explicit user actions with layered registration,
argument handling, project/tool preflight, progress, cancellation, and
completion. The standard structured turn handler delegates one turn to
~magent-runtime-submit~; its turn spec separates prompt text, request context,
agent/skill selection, and ledger metadata. Advanced trusted packages may own
a longer async invocation without receiving ACP or queue internals. The bundled
...
into the command registry. Its handler still activates the original skill.
Project and user adapters override package/bundled definitions by layer, while
core controls are reserved. Project adapters remain keyed by canonical project
...
Magent also has an Elisp-level internal command registry in ~magent-internal-command.el~.
Commands registered with ~magent-internal-command-register~ are invoked by explicit
...
- A ~command~ is an explicit slash invocation implemented by trusted Elisp. A
