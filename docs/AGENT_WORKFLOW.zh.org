...
Magent 现在把 agent loop 建模为显式的 ~thread -> turn -> item~ ledger。这个 ledger 是 agent workflow state 的 source of truth。旧 session ~messages~ 仍然保留，但只作为 gptel prompt construction 和迁移的 derived projection。
...
- Magent loop 自己负责 tool dispatch、continuation outcome、abort 和 Emacs UI rendering；turn layer 遵循 Codex-style continuation：tool output 触发下一次 sampling，assistant completion 结束 turn，即使 assistant text 为空。
...
旧 ~messages~ 是 prompt reuse 和迁移用的 projection。加载时，Magent 优先 replay ~snapshot + journal~ ；若不存在则把 legacy ~messages~ 迁移成 thread ledger。
...
10. 如果 post-tool continuation 返回空 assistant text，Magent 会进行一次禁用 provider tools 的 final-response retry。若该 retry 只有 reasoning 而没有 text，再进行一次更严格且禁用 reasoning 的 retry；严格 retry 仍为空时 turn 失败。
...
13. ~magent-max-sampling-requests~ 是可选兼容 safety guard，默认关闭。Codex 没有 per-turn sampling cap；用户显式启用时，Magent 在到达上限时用一次禁用 provider tools 的 final request 替代后续 continuation。
...
受支持的 frontend 是 agent-shell。 ~magent-agent-shell.el~ 创建由 ~magent-acp.el~ 实现的 in-process ACP client。ACP session/prompt request 把 registered slash command 分派给 ~magent-command.el~ ，普通 prompt 则直接提交 ~magent-runtime-api.el~ ；request 会保持 pending，直到对应 command invocation 或普通 runtime turn completed、failed 或 cancelled。Command-owned turn 和 workflow step 使用同一个 runtime API。Runtime 发出 Magent-native observer events； ~magent-acp.el~ 转换为 ACP ~session/update~ 消息。
...
~M-x~ wrapper 进入同一个 ~magent-command~ invocation lifecycle，不会形成另一套会话
