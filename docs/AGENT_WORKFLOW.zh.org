...
- ~journal~ ：内存中的 append-only event log，记录 ~turn-queued~ 、 ~turn-started~ 、 ~item-started~ 、 ~item-completed~ 、 ~turn-completed~ 等生命周期事件。Session 文件只保留由 ~magent-session-journal-max-events~ 控制的最近有界尾部；tool/permission 审计由 ~magent-audit.el~ 单独负责。
- ~snapshot~ ：materialized full thread state。它保存当前 thread、turns 和 items，避免每次 resume 都从头 replay。

当前 session schema（version 6）同时写入两者：
...
加载器只接受完整、字段精确的 version 6 ~snapshot + journal~。缺少 ledger、版本不同、字段未知或嵌套 shape 无效的文件都会被拒绝；没有旧格式迁移或双写路径。
...
因此 ~snapshot~ 是快速恢复点，持久化 ~journal~ 是有界恢复尾部，两者不可互换。
