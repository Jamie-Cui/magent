...
10. 如果 post-tool continuation 返回空 assistant text，Magent 记录空 assistant message 并完成 turn。空消息保存在 transcript 中，但不会回灌到后续 prompt。
...
13. ~magent-max-sampling-requests~ 是可选兼容 safety guard，默认关闭。Codex 没有 per-turn sampling cap；用户显式启用时，Magent 到达上限后失败 turn，而不是强行追加 no-tool final answer。
