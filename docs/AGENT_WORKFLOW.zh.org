...
受支持的 frontend 是 agent-shell。 ~magent-agent-shell.el~ 创建由 ~magent-acp.el~ 实现的 in-process ACP client。ACP session/prompt request 提交到 ~magent-runtime-api.el~ ，并保持 pending 直到对应 runtime turn completed、failed 或 cancelled。Runtime 发出 Magent-native observer events； ~magent-acp.el~ 转换为 ACP ~session/update~ 消息。
...
agent-shell + ACP 路径是完整的 UI projection 和公共命令面；共享 runtime 行为保持
frontend-neutral。
