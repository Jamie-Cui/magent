...
- *Slash commands* 在当前 agent-shell 对话中调用 Elisp-native Action；大多数
  内置 prompt Action 声明一次普通 agent turn。
...
| ~/memory-init~ | 建立 Emacs profile memory | 在独立 Action session 中确认并执行 |
| ~/memory-refresh~ | 刷新 managed profile memory | 在独立 Action session 中保留 user notes |
...
返回当前 session agent 的有效 catalog view。每一行显示 permission key、最终
decision、tool 是否暴露、rule 或 session override 来源、resource sub-rules、
approval policy，以及执行发生在 host、child Emacs 还是 live Emacs。
...
- ~magent-workflow-agent-turn~ 执行中间 model turn，默认返回文本。它支持 ~:agent~、
  ~:skills~、~:buffers~、~:append-argument-p~、~:tools~、
  ~:request-context~、~:resource-blocks~ 和 ~:result~。
- ~magent-workflow-answer~ 支持相同的 model 选项，但它是 terminal Step：流式输出
...
- ~magent-workflow-process~ 只运行由非空 string 组成的 argv list。它会捕获调用点
  ...
  exit status、duration 和 timeout state 的 ~magent-action-process-result~。
- ~magent-workflow-callback~ 用来适配已有的异步 Elisp API。Start function 接收
...
| Emacs 命令                     | 工作流 | LLM 行为 |
|--------------------------------+--------+----------|
| ~magent-action-run-memory-init~       | 建立初始 Emacs profile memory | ~magent-memory-use-llm~ 非 nil 时使用当前 gptel provider |
| ~magent-action-run-memory-refresh~    | 刷新 managed profile memory | ~magent-memory-use-llm~ 非 nil 时使用当前 gptel provider |
| ~magent-action-run-memory-clear~      | 停用并清理 managed memory | 本地操作，不请求 LLM 总结 |
| ~magent-action-run-doctor~            | 诊断 Magent 和当前 runtime 状态 | 发送一次经过脱敏且不带 tools 的分析请求 |

** Memory 初始化与刷新

~M-x magent-action-run-memory-init~ 发现有边界的 Emacs 配置来源，展示 scan plan 供用户
确认，然后写入 managed profile memory 文件。~M-x magent-action-run-memory-refresh~
重新扫描并保留 user notes。
...
~M-x magent-action-run-memory-clear~ 经确认后停用 managed profile 并清理生成内容，
...
Action-session viewer 优先显示最终结果，并默认折叠详细 activity。Cancellation
