...
- ~magent-workflow-agent-turn~ 执行中间 model turn，默认返回文本。它支持 ~:agent~、
  ~:skills~、~:buffers~、~:append-argument-p~、~:tools~、~:effort~、~:thinking~、
...
~M-x magent-action~ 在执行未批准的文件前询问。信任记录覆盖完整文件名和内容集合，
...
| Emacs 命令                        | 用途 |
|-----------------------------------+------|
| ~magent-action~                | 使用标准 minibuffer 补全选择并运行 Action |
...
Action-session viewer 优先显示最终结果，默认折叠详细 activity，并使用 Org 的标题、
正文标记和代码块语法高亮。~TAB~ 切换当前节，~S-TAB~ 切换 Activity，~q~ 退出。Cancellation
按 session 隔离，不会取消其他 Magent 对话所属的工作。

~M-x magent-action~ 列出当前 scope 中具有 interactive exposure 的 Actions，
附带描述和选择历史。它使用与 ~M-x~ 相同的补全界面，包括已启用的
Vertico/posframe。选择后按 ~RET~ 启动；~C-u M-x magent-action~ 可以输入参数。
工具权限沿用现有 agent/session 规则。可以在个人配置中绑定快捷键，例如：
