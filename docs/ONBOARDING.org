...
- ~magent-ui.el~ — UI customization/faces and backend routing
...
~magent-compose-mode~ buffer. See ~docs/UI_BACKENDS.org~ before changing this
boundary.
...
Read ~docs/UI_BACKENDS.org~, then ~magent-ui.el~, ~magent-agent-shell.el~, and
...
** User Interface
- *magent-ui.el* — UI customization/faces, thin backend router, log/event sinks, and compatibility shims
...
- *magent-ui-legacy.el* — Legacy workspace/compose buffers, private submission queue, streaming summaries, transcript/detail views, transient menu
