...
Skills load from: (1) built-in ~skills/~, (2) user ~~/.emacs.d/magent/skills~, (3) legacy ~~/.emacs.d/magent-skills/~ when present, and (4) project ~.magent/skills/~.  A skill can also declare ~capability: true~ with resolver metadata such as ~modes~, ~features~, ~files~, ~keywords~, and ~disclosure~; Magent turns that metadata into a capability that activates the same skill.
