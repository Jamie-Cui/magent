...
#+magent_lang: en
#+magent_alt_url: /ONBOARDING.zh.html
...
- *Primary Dependency:* [[https://github.com/karthink/gptel][gptel]] (handles all LLM communication)
...
- ~magent-ui.el~ — Retained unsupported compatibility code
...
registry, skills) happens on demand.
...
- ~magent-command.el~ — Session-backed internal workflows invoked by ~magent-run-*~ commands
- ~magent-doctor.el~ — Trusted read-only probes and one sanitized tool-free analysis request
- ~magent-redaction.el~ — Fail-closed redaction for Magent-owned outbound diagnostics and memory
...
*What it does:* Maintains ledger-backed conversation history scoped by project, persists to ~~/.emacs.d/magent/sessions/~ by default, stores durable child-agent jobs under ~agent-jobs~, and activates or unloads project-local overlays as scope changes. The ACP frontend submits work through ~magent-runtime-api.el~; ~magent-runtime-queue.el~ owns queued/active turn state and session-scoped cancellation. Internal maintenance workflows such as ~magent-run-doctor~ and ~magent-run-memory-init~ run in isolated sessions under ~magent-session-directory/internal~ instead of agent-shell slash commands. Doctor is a direct probe pipeline with no model tools; read [[file:DOCTOR.org][DOCTOR.org]] before changing its data boundary or extension API.
...
*Key Files:*
- ~magent-tools.el~ — 15 ~gptel-tool~ structs: read_file, write_file, write_repo_summary, edit_file, grep, glob, bash, emacs_eval, spawn_agent, send_agent_message, wait_agent, list_agents, close_agent, skill_invoke, web_search
...
agent-shell is the only supported frontend.
...
- ~magent-ui.el~ / ~magent-ui-legacy.el~ — Retained unsupported UI code; not a public interface
...
corresponding Magent turn completes. See ~docs/UI_BACKENDS.org~ for the
unsupported legacy-file boundary before changing this layer.
...
also be run as command-like actions; built-in
~init~ initializes or refreshes the project root ~AGENTS.md~, similar to Codex
~/init~, and accepts optional extra instructions.
...
~magent-runtime-api.el~. Key insight: agent-shell is the only supported
interactive experience. Read ~magent-ui.el~ or ~magent-ui-legacy.el~ only when
removing or isolating unsupported compatibility code.
...
- *magent-tools.el* — 15 tool implementations as gptel-tool structs
...
- *magent-ui.el* / *magent-ui-legacy.el* — Retained unsupported UI code
...
Magent packages runtime data as well as Elisp. ~prompt/~ and ~skills/~
...
policy, not only ~prompt/system.org~. Every bundled Org prompt must also appear
exactly once in ~prompt/manifest.txt~; the unit suite checks the manifest against
...
*Why it's complex:* The supported frontend spans ~magent-agent-shell.el~, ~magent-acp.el~, ~magent-runtime-api.el~, and ~magent-runtime-queue.el~. Unsupported compatibility code remains in ~magent-ui.el~ and ~magent-ui-legacy.el~.
...
~magent-agent-shell.el~. Do not add user-facing behavior to unsupported UI
files.
...
- *Interactive help:* ~M-x magent-run-doctor~ for self-diagnostics
...
2. *Try the examples* — Run ~M-x magent-agent-shell-dwim~
