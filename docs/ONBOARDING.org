...
*Key Files:*
- ~magent-tools.el~ — canonical catalog of 16 ~gptel-tool~ structs: read_file, write_file, edit_file, grep, glob, bash, emacs_read, emacs_eval, emacs_eval_live, read_tool_output, spawn_agent, send_agent_message, wait_agent, list_agents, close_agent, web_search
...
belong in trusted Actions or first-class catalog tools. Approval gates
...
request-local file resources, trusted blocks from
~magent-context-provider-functions~, and the small set of explicit or
capability-selected instruction skills. Discovery never leaves the
...
and utility agents. Context providers are trusted local Elisp, but supporting
data they embed cannot override current user instructions or live
Emacs/repository state. Text read from
...
- Use an ~Action~ for executable trusted Elisp. It may be projected as a slash
  command, an interactive command, or both.
...
- Use a ~skill~ for reusable instructions that can be selected explicitly or
  automatically. ~/$name~ is the ACP projection; it does not create an Action.
...
- *magent-tools.el* — Canonical catalog of 16 ~gptel-tool~ implementations
...
the directory. Keep this map aligned with every production file in
~source-files.txt~.
...
Use exact permission group keys such as ~read~, ~write~, ~edit~, and ~agent~.
Unknown keys and non-mapping permission forms are rejected.
