...
如果你在调试 agent lifecycle 或 subagent 行为，先看 `docs/AGENT_JOBS.md`。Magent 当前通过 `spawn_agent`、`send_agent_message`、`wait_agent`、`list_agents` 和 `close_agent` 使用 durable child-agent jobs；旧的一次性 `delegate` tool 不属于当前 surface。不要把缺少 sandbox parity 诊断成 Magent bug；Codex sandbox behavior 是有意排除的范围。

## 常见问题

### GitHub Actions 失败

#### melpazoid 找不到 `prompt.org`
...
**原因：** MELPA-style packaging 只复制 recipe 匹配的文件。Magent 打包后仍然需要 bundled runtime data：`prompt.org`、`skills/` 和 `capabilities/`。
...
2. 复读 `docs/AGENT_JOBS.md` 中的 job lifecycle contract。
