...
~/doctor~ 和 ~M-x magent-command-run-doctor~ 用于诊断 Magent、Emacs runtime
和当前项目状态，但不会把通用 Emacs 或 shell 工具交给 LLM。两个入口执行同一个
command spec，并在 ~magent-session-directory/commands/doctor~ 下创建独立
command session。
...
queue。它可以和普通对话并发，但不会把 command session 安装成用户当前
session。
...
脱敏和限长的数据会进入 command session 和 provider request。
...
使用 ~M-x magent-command-list-sessions~ 查看结果。Viewer 会优先显示最终诊断，
默认折叠 probe activity；~TAB~ 切换当前 section，~S-TAB~ 切换全部 activity。

使用 ~M-x magent-command-cancel~ 取消活动 Doctor。取消会终止 probe
process 或 abort 直接 gptel request，将 command session 标记为 cancelled，
并保持用户当前 session 不变。
