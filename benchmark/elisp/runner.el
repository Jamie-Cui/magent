...
(defun magent-benchmark--capture-usage (original loop event)
  ...
  (let* ((type (and (fboundp 'magent-sampling-event-type)
                    (magent-sampling-event-type event)))
         (usage (or (and (fboundp 'magent-sampling-event-usage)
                         (magent-sampling-event-usage event))
                    (and (memq type '(tool-call-batch-end error))
                         (fboundp 'magent-sampling-event-metadata)
                         (plist-get (magent-sampling-event-metadata event) :tokens)))))
