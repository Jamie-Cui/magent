;;; magent-transcript-context.el --- Structured transcript context for Magent  -*- lexical-binding: t; -*-
...
(defun magent-transcript-context-items (session)
  "Return SESSION's structured response items."
  (and session (magent-session-context-items session)))

(defun magent-transcript-context-record-message (session role content &optional phase)
...
(defun magent-transcript-context-record-tool-call (session call)
...
(defun magent-transcript-context-record-tool-result (session result)
...
(defun magent-transcript-context-message-items (session)
  "Return only message items from SESSION's structured context."
  (cl-remove-if-not
   (lambda (item) (eq (magent-response-item-type item) 'message))
   (magent-transcript-context-items session)))

(defun magent-transcript-context-approx-token-count (session)
  "Return a coarse token estimate for SESSION's structured context."
  (let ((chars 0))
    (dolist (item (magent-transcript-context-items session))
      (cl-incf chars
               (length (format "%s%s%s"
...
(provide 'magent-transcript-context)
;;; magent-transcript-context.el ends here
