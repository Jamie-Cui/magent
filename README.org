...
| Definition             | Trusted Elisp registered with ~magent-command-register~                 | A data-only ~SKILL.md~ file                                        |
...
| Project trust boundary | Command Workflows are trusted installed Elisp                         | Project skill Markdown never executes code                       |
...
while handling requests. Use both when an explicit command
...
|--------------------+-------------+---------------------------------------------------------|
| ~read_file~          | no          | Read saved file contents from disk                      |
