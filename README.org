...
instructions such as focusing on tests.  Agent-shell also advertises seven
bundled slash commands: the ~/explain~, ~/fix~, ~/init~, ~/review~, and ~/test~
skill commands plus the ~/compact~ and ~/clear~ session controls.
...
9. *Skills* (~magent-skills.el~): Two skill types -- ~instruction~ (markdown injected into the system prompt) and ~tool~ (invoked via ~skill_invoke~). Instruction skills can be selected from agent-shell as one-shot request context, skills that declare ~default-prompt~ can be run as command-like agent-shell actions such as ~init~, and skills that declare ~capability: true~ can auto-activate through the capability resolver. The module contains the registry, built-in ~skill-creator~ definition, file-based skill loading, and interactive inspection commands. Skills load from: (1) built-in ~skills/~ directory bundled with magent, (2) user directory =~/.emacs.d/magent/skills=, (3) legacy =~/.emacs.d/magent-skills= when present, and (4) project-local ~.magent/skills/<name>/SKILL.md~.
   ...
   ~test~, and ~explain~ are command-style skills without automatic capability
   activation.
...
12. *Audit* (~magent-audit.el~): Persistent JSONL audit logging for permission prompts and decisions plus sensitive actions such as ~bash~, ~emacs_eval~, ~write_file~, ~edit_file~, and child-agent lifecycle tools. Payloads are redacted to metadata and truncated previews.
