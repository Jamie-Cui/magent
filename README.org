...
Formal legacy session commands are selected from the transient menu instead of
typed into compose or bound as a wide set of default ~C-c m~ shortcuts. Open the
menu with ~C-c m ?~ from any Magent-enabled buffer or ~?~ from the legacy
~*magent*~ workspace. The top-level menu routes agent, skill, capability,
session, log, health, and buffer commands through small Magit-style submenus.
...
Instruction skills are selected from the transient menu as one-shot context for
the next request.  Use ~? s s~ to toggle an instruction skill, and ~? s K~ to
clear the selected skills.  Selected skills are scoped to the current Magent
session scope and are shown in the legacy ~*magent*~ workspace header-line until
the next prompt is submitted.

Skills with a ~default-prompt~ can also be run as command-like transient
actions.  For example, ~? i~ runs the built-in ~init~ skill from the top-level
menu, while ~? s i~ runs the same action from the Skills submenu.  It
initializes or refreshes the project's root ~AGENTS.md~ and prompts for optional
extra instructions such as focusing on tests.  Compose buffers no longer parse
~@skill~ or ~@clear~ control syntax; that text is sent to the model as ordinary
prompt text.
