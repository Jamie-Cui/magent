...
- *Read-only workspace + compose UI* backed by the thread ledger
...
Formal session commands are selected from the transient menu instead of typed
into compose or bound as a wide set of default ~C-c m~ shortcuts.  Open the menu
with ~C-c m ?~ from any Magent-enabled buffer or ~?~ from the ~*magent*~
workspace.  The top-level menu keeps common request actions visible and routes
agent, skill, capability, session, log, health, and buffer commands through
small Magit-style submenus.

In the ~*magent*~ output buffer: ~TAB~ and ~S-TAB~ fold sections, ~?~ opens the transient menu, ~C-c C-o~ opens compose directly, and ~C-c C-c~ opens compose when idle or confirms interrupt while a request is running.  Compose is plain prompt text; session commands and one-shot skill selection live in the transient menu, and the workspace header-line shows the current scope, agent, thread status, request state, queue length, session id, and selected skills for the next request.
...
session scope and are shown in the ~*magent*~ workspace header-line until the
...
Child jobs are stored in the parent session under ~agent-jobs~ with status, prompt, metadata, result/error, and transcript state. The parent workspace shows compact ~Agent ...~ lifecycle rows, while the full child transcript is available through ~magent-show-agent-transcript~ or ~? S j~ in the transient menu. See [[file:docs/AGENT_JOBS.md][docs/AGENT_JOBS.md]] for the implementation model and boundaries.
...
12. *UI* (~magent-ui.el~): The ~*magent*~ workspace derives from ~special-mode~ and renders a compact read-only oldest-first timeline of retained non-dropped ledger turns. Prompt editing lives in scope-specific ~magent-compose-mode~ buffers; ~C-c C-c~ submits. Compose is plain prompt text, while formal commands and one-shot skills are selected from the Magit-style transient menu. The workspace header-line is the single status surface for scope, agent, thread status, request state, queue length, session id, and pending skills; tool calls, child-agent events, and reasoning render as compact status rows/fragments rather than Org source blocks.
...
|-- magent-ui.el                   # Read-only workspace and compose UI
