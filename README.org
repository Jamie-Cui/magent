...
through TRAMP.  Only ~bash~ and ~grep~ start project-host processes, and they
fail explicitly if their configured executable is unavailable there; Magent
never falls back to running them in a local home directory.
...
| ~magent-grep-program~                          | ~"rg"~                              | Ripgrep executable resolved on the local or TRAMP project host                                 |
