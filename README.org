...
In the ~*magent*~ output buffer: ~TAB~ and ~S-TAB~ fold sections, ~?~ opens the transient menu outside the editable input area, and ~C-c C-c~ opens compose when idle or confirms interrupt while a request is running.  Submit ~@clear~ as the whole input to clear the current session context, similar to Codex ~/clear~.
...
12. *UI* (~magent-ui.el~): The ~*magent*~ workspace derives from ~special-mode~ and renders a compact read-only projection of current/recent ledger turns. Prompt editing lives in scope-specific ~magent-compose-mode~ buffers; ~C-c C-c~ submits. Tool calls, child-agent events, and reasoning render as compact status rows/fragments rather than Org source blocks.
