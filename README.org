...
The current active architecture goal is to compare Magent's agent workflow with the local Codex checkout at =~/proj/codex= and move Magent toward Codex-style collaborative agent behavior where it fits Emacs.

The planned direction is a durable child-agent lifecycle -- spawn, message, wait, list, resume/inspect, and close -- on top of a Magent-owned agent loop that keeps using ~gptel-request~ for provider integration. Breaking changes are allowed for this work, including replacing ~delegate~ directly once the new tool surface exists. Codex sandbox behavior is explicitly out of scope. The working plan and recovery checklist live in [[file:docs/plans/2026-05-30-codex-agent-workflow-alignment.md][docs/plans/2026-05-30-codex-agent-workflow-alignment.md]].
