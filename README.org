...
In the ~*magent*~ output buffer: ~TAB~ and ~S-TAB~ fold sections, ~?~ opens the transient menu outside the editable input area, ~C-c C-c~ submits input, and ~C-g~ interrupts.
