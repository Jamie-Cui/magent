...
- *15 built-in tools*: file operations, repository summaries, shell, grep, glob, web search, Emacs eval, durable child-agent jobs, and skill invocation
...
12. *Audit* (~magent-audit.el~): Persistent JSONL audit logging for permission prompts and decisions plus sensitive actions such as ~bash~, ~emacs_eval~, ~write_file~, ~write_repo_summary~, ~edit_file~, and child-agent lifecycle tools. Free-text command, prompt, source, message, and result bodies are never persisted; records contain scalar request attribution, lengths, statuses, and normalized path markers only.

13. *UI backend* (~magent-ui.el~, ~magent-agent-shell.el~, ~magent-acp.el~): ~magent-ui.el~ owns UI customization/faces plus the thin command router and compatibility layer. The recommended backend is agent-shell through an in-process ACP adapter; ACP prompt requests submit to the runtime API and stay pending until the turn completes. See [[file:docs/UI_BACKENDS.org][docs/UI_BACKENDS.org]].
...
|   |-- magent-config.el           # UI-neutral runtime and feature configuration
...
|   |-- magent-skills.el           # Skill registry, file loading, and commands
|   |-- magent-ui.el               # UI configuration/faces, backend router, and shims
|   |-- magent-modeline.el         # Mode-line formatting and status installation
|   |-- magent-ui-legacy.el        # Legacy workspace, compose buffer, and private queue
