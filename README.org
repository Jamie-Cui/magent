...
5. *LLM Integration* (via gptel): All LLM communication is handled by [[https://github.com/karthink/gptel][gptel]]. Provider, model, and API key configuration is managed entirely by gptel.
...
9. *Audit* (~magent-audit.el~): Persistent JSONL audit logging for permission prompts and decisions plus sensitive actions such as ~bash~, ~emacs_eval~, ~write_file~, ~edit_file~, and ~delegate~. Payloads are redacted to metadata and truncated previews.
...
11. *UI* (~magent-ui.el~): The ~*magent*~ buffer derives from ~org-mode~. It uses in-buffer input with ~* [USER]~ sections. Tool calls render as ~#+begin_tool~/~#+end_tool~ blocks; reasoning blocks as ~#+begin_think~/~#+end_think~.

12. *Markdown-to-Org* (~magent-md2org.el~): Converts markdown assistant output to org-mode format for rendering.

13. *Frontmatter* (~magent-frontmatter.el~): Shared YAML frontmatter parser for agent and skill files. Supports booleans, numbers, quoted strings, and comma-separated lists.
...
|-- magent-skill-file.el           # Skill file loader (SKILL.md files plus built-in skills/ dir)
