...
11. *Session* (~magent-ledger.el~, ~magent-session.el~): Thread-ledger backed conversation history with per-project active sessions, global fallback outside projects, JSON persistence, internal command session storage, and durable restore from ledger snapshots plus journals.
