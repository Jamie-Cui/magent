...
Child jobs are stored in the parent session under ~agent-jobs~ with status, prompt, metadata, result/error, and transcript state. The parent buffer shows compact ~#+begin_agent~ lifecycle blocks, while the full child transcript is available through ~magent-show-agent-transcript~ (~C-c m j~). See [[file:docs/AGENT_JOBS.md][docs/AGENT_JOBS.md]] for the implementation model and boundaries.
...
14. *Markdown-to-Org* (~magent-md2org.el~): Converts markdown assistant output to org-mode format for rendering.
