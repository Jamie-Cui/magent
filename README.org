...
In the ~*magent*~ output buffer: ~TAB~ and ~S-TAB~ fold sections, ~?~ opens the transient menu outside the editable input area, ~C-c C-o~ opens compose directly, and ~C-c C-c~ opens compose when idle or confirms interrupt while a request is running.  Submit ~@clear~ as the whole input to clear the current session context, similar to Codex ~/clear~.
...
In the Magent input area, type ~@skill-name~ to explicitly activate an
...
Child jobs are stored in the parent session under ~agent-jobs~ with status, prompt, metadata, result/error, and transcript state. The parent workspace shows compact ~Agent ...~ lifecycle rows, while the full child transcript is available through ~magent-show-agent-transcript~ (~C-c m j~). See [[file:docs/AGENT_JOBS.md][docs/AGENT_JOBS.md]] for the implementation model and boundaries.
