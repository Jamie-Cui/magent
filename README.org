...
The old one-shot ~delegate~ tool has been replaced by ~spawn_agent~, ~send_agent_message~, ~wait_agent~, ~list_agents~, and ~close_agent~. Codex sandbox behavior is intentionally out of scope. The final architecture is documented in [[file:docs/AGENT_JOBS.org][docs/AGENT_JOBS.org]].
...
Child jobs are stored in the parent session under ~agent-jobs~ with status, prompt, metadata, result/error, and transcript state. The UI shows compact lifecycle updates, while the full child transcript is available through ~magent-show-agent-transcript~ or ~? S j~ in the legacy transient menu. See [[file:docs/AGENT_JOBS.org][docs/AGENT_JOBS.org]] for the implementation model and boundaries.
...
10. *Session* (~magent-ledger.el~, ~magent-session.el~): Thread-ledger backed conversation history with per-project active sessions, global fallback outside projects, and JSON persistence. ~buffer-content~ is legacy data only; workspace restore renders from the ledger.
...
14. *Markdown-to-Org* (~magent-markdown-to-org.el~): Legacy markdown-to-org conversion helper kept outside the live workspace path.
...
|-- magent-legacy-queue.el         # Legacy UI submission queue compatibility layer
...
|-- magent-markdown-to-org.el      # Legacy markdown to org-mode compatibility helpers
