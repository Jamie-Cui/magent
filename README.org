...
~M-x magent-agent-shell-run-command~. Agent-shell advertises eight bundled
commands: ~/explain~, ~/fix~, ~/init~, ~/review~, ~/summarize~, ~/test~,
~/compact~, and ~/clear~. The six prompt commands own package Org resources and
...
Project and user skill adapters may override package commands, while the two
core session controls are reserved. See [[file:docs/COMMANDS.org][docs/COMMANDS.org]]
