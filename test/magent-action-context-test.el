...
(ert-deftest magent-context-picker-management-keeps-origin-and-ignores-prefix ()
  "Every management entry calls its command without starting an Action."
  (let ((magent-action--registry nil)
        (current-prefix-arg '(4)))
    (magent-action-register
     "org-only" :exposure '(interactive) :modes '(major org-mode)
     :session-policy 'isolated :workflow (iter-lambda (_) nil))
    (dolist (choice '(("manage: sessions" . magent-action-list-sessions)
                      ("manage: cancel" . magent-action-cancel)
                      ("manage: reload-project" . magent-action-reload-project)
                      ("manage: clear-results" . magent-action-mode-line-clear-results)))
      (with-temp-buffer
        (let ((origin (current-buffer))
              (elsewhere (generate-new-buffer " *action-menu-elsewhere*"))
              called)
          (unwind-protect
              (cl-letf (((symbol-function 'magent-runtime-ensure-initialized) #'ignore)
                        ((symbol-function 'magent-runtime-prepare-context) #'ignore)
                        ((symbol-function 'magent-runtime-context-scope)
                         (lambda () "/tmp/action-menu-project/"))
                        ((symbol-function 'magent-action-session-active-invocations)
                         (lambda (&optional _) '(active)))
                        ((symbol-function 'magent-action-mode-line-results-p)
                         (lambda () t))
                        ((symbol-function 'completing-read)
                         (lambda (_prompt table &rest _)
                           (should-not (member "org-only" (all-completions "" table)))
                           (should (member (car choice) (all-completions "" table)))
                           (set-buffer elsewhere)
                           (car choice)))
                        ((symbol-function 'read-string)
                         (lambda (&rest _) (ert-fail "Unexpected Action argument")))
                        ((symbol-function 'magent-action-run)
                         (lambda (&rest _) (ert-fail "Unexpected Action dispatch")))
                        ((symbol-function (cdr choice))
                         (lambda ()
                           (interactive)
                           (should (eq (current-buffer) origin))
                           (should-not current-prefix-arg)
                           (setq called t))))
...
(ert-deftest magent-context-picker-management-follows-available-work ()
  "Management entries reflect scope, cancellable work, and retained results."
  (let* ((magent-action--registry nil)
         (magent-action-session--active-invocations (make-hash-table :test #'equal))
         (magent-action-mode-line--invocations (make-hash-table :test #'eq))
         (invocation (magent-action-invocation-create :id "menu-task"))
         (scope 'global)
         candidates)
    (cl-letf (((symbol-function 'magent-runtime-ensure-initialized) #'ignore)
              ((symbol-function 'magent-runtime-prepare-context) #'ignore)
              ...
              ((symbol-function 'completing-read)
               (lambda (_prompt table &rest _)
                 ...
                 (signal 'quit nil))))
      (cl-flet ((inspect-menu ()
                  (condition-case nil (magent-action) (quit nil))))
        (inspect-menu)
        (should (equal candidates '("manage: sessions")))
        (setq scope "/tmp/action-menu-project/")
        (puthash "menu-task" invocation magent-action-session--active-invocations)
        (magent-action-mode-line--track invocation)
        (inspect-menu)
        (should (equal candidates '("manage: sessions" "manage: reload-project")))
        (setf (magent-action-invocation-current-step invocation)
              (magent-action-step-create :type 'callback))
        (inspect-menu)
        (should (equal candidates '("manage: sessions" "manage: cancel"
                                   "manage: reload-project")))
        (dolist (status '(failed completed))
...
(ert-deftest magent-context-picker-defers-project-approval-to-reload ()
  "Browsing stays prompt-free; selecting reload explicitly approves sources."
  (magent-context-test--with-project
    (let ((selection "manage: sessions")
          (prompts 0)
          sessions-opened)
      (cl-letf (((symbol-function 'magent-runtime-ensure-initialized) #'ignore)
                ((symbol-function 'magent-runtime-context-scope) (lambda () scope))
                ((symbol-function 'magent-runtime-prepare-context)
                 (lambda (target) (magent-action-load-project-scope target)))
                ((symbol-function 'completing-read)
                 (lambda (_prompt table &rest _)
                   (should (= prompts 0))
                   (should-not (member "local" (all-completions "" table)))
                   selection))
                ((symbol-function 'yes-or-no-p)
                 (lambda (&rest _)
                   (should (equal selection "manage: reload-project"))
                   (cl-incf prompts)
                   t))
                ((symbol-function 'magent-action-list-sessions)
                 (lambda () (interactive) (setq sessions-opened t))))
        (magent-action)
        (should sessions-opened)
        (should (= magent-context-test--loads 0))
        (should-not (file-exists-p magent-action-project-trust-file))
        (setq selection "manage: reload-project")
        (magent-action)
        (should (= prompts 1))
        (should (= magent-context-test--loads 1))
        (should (magent-action-get "local" scope 'interactive))
        ;; Approved sources are offered on the next menu without another prompt.
        (cl-letf (((symbol-function 'completing-read)
                   (lambda (_prompt table &rest _)
                     (should (member "local" (all-completions "" table)))
                     "manage: sessions")))
          (magent-action))
        (should (= prompts 1))
        (should (= magent-context-test--loads 1))))))

(ert-deftest magent-context-picker-management-rejects-dead-origin ()
  "A vanished origin must not redirect a management command to another buffer."
  (let ((origin (generate-new-buffer " *action-menu-origin*"))
        (magent-action--registry nil))
    (unwind-protect
          ...
          (cl-letf (((symbol-function 'magent-runtime-ensure-initialized) #'ignore)
                    ...
                    ((symbol-function 'completing-read)
                     (lambda (&rest _)
                       ...
                       "manage: sessions"))
                    ((symbol-function 'magent-action-list-sessions)
                     (lambda () (interactive) (ert-fail "Unexpected management dispatch"))))
